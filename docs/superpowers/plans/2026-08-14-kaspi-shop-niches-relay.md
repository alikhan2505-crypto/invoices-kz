...
- Route auth: `request` and `result` require `is_admin` (401 otherwise, via a `requireAdmin` helper matching `src/app/api/kaspi/admin-stats/route.ts`'s existing pattern) — not just any authenticated user, because `checkId` has no owner column and "any authenticated user" would let one invoices.kz customer read another's niche-check result (an IDOR). `deliver` requires the `x-kaspi-shop-cron-secret` header to match `KASPI_SHOP_CRON_SECRET` (401 otherwise) — it is called by the GitHub Actions runner, never by a browser.
