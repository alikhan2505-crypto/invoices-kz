...
**Open question, not yet confirmed — flagged for Task 1's live research, not assumed:** whether an order entry's `product.code` (from the orders GraphQL API) and a catalog offer's `sku` (from the separate `bff/offer-view/list` catalog REST endpoint that populates `kaspi_shop_tracked_products.kaspi_sku` at connect time) are actually the same identifier space. Both are named consistently with "SKU" across this codebase's existing code, but the two values have never been directly compared against each other from a real order — they come from two different Kaspi API surfaces. Task 1 must confirm this join key live (fetch one real order's items, fetch the matching product from `kaspi_shop_tracked_products`, confirm the codes match) before any aggregation code is written — if they turn out to be different identifier spaces, per-product revenue would silently attribute to nothing for every product, exactly the kind of silent failure this project has repeatedly hit and fixed elsewhere (`sessionExpired` propagation, `size:50`, etc.). Per-product COGS total is `cogs_amount * unitsSold` when `cogs_amount` is set, or `null` (propagated, not zeroed) when it isn't.
