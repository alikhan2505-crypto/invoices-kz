...
Новый вебхук-хендлер (обобщение `src/app/api/instagram/webhook/route.ts`): входящее сообщение → находим `ai_agent_channel_connections` по `external_account_id` из вебхука → находим агента → шаблон агента (мгновенно, бесплатно) → нет совпадения → `generateAiReply` с контекстом агента.
