...
typography:
  ...
  marketing-display:
    ...
    fontSize: "clamp(30px, 5.6vw, 66px)"
    fontWeight: 700
  marketing-body:
...
- **Documented exception (2026-09-05, `KaspiShopStoreSwitcher.tsx`):** в тесной многокнопочной плашке (соседние кнопки на расстоянии <4px), где расширение одной тач-зоны до 44px либо раздувает всю плашку, либо перекрывает зону соседней кнопки, допустимо оставить кнопку меньше 44px — промах между кнопками хуже маленькой цели. Применять только когда обе альтернативы (раздуть плашку целиком / рискнуть перекрытием) хуже статус-кво; не использовать как общую отговорку для неисправленных мест.
...
## 7. Маркетинговые поверхности

Решение основателя 21.09.2026 (макет разделения на поддомены). Система выше описывает **кабинеты** — рабочие экраны, где человек делает дело. Стартовая страница `invoices.kz` и лендинги продуктов — другая работа: привлечь внимание и подсказать, что выбрать. Для них принят отдельный, более выразительный слой. Он **не отменяет** правила выше и не просачивается в кабинеты.

**Что это за поверхности:** стартовая страница со всеми продуктами и входом, лендинг каждого продукта (`kaspi.`, `api.`, `agent.`, `salon.`, `docs.`). Всё остальное — кабинеты, страницы документов, планировщик — остаётся в системе разделов 1–6.

### Что здесь разрешено
- **Шрифты:** Unbounded (заголовки, только крупно, 700), Onest (текст), Caveat (рукописная пометка, не больше одной на экран). В кабинетах по-прежнему только Geist.
- **Радиусы:** плитки продуктов `tile` (28px), стикеры `sticker` (18px), кнопки и чипы — `full`. Кабинеты остаются на шкале 8–16px.
- **Цвет продукта:** у каждого продукта один насыщенный плоский цвет и один цвет текста на нём (`mk-*` в шапке файла). Цвет живёт только на плитке и лендинге этого продукта.
- **Иллюстрации:** плоские CSS/SVG-фигуры вместо фото и стоков, по одной на продукт, по его предмету (листы и печать — счета, коробка и ценник — Kaspi, скобки `{ }` — API, два диалога — агент, календарь — салон, ягоды — Wildberries).
- **Фон стартовой:** `hub-mist` в светлой теме, `hub-night` в тёмной; мягкие размытые пятна и гигантская контурная «6» (по числу продуктов).
- **Движение:** лёгкое покачивание фигур, смещение за курсором внутри плитки, приподнимание плитки при наведении. Всё отключается при `prefers-reduced-motion`. Контент виден сразу, без ожидания прокрутки.

### Named Rules
**The Two-Layer Rule.** Выразительность заканчивается на пороге кабинета. Как только человек вошёл в продукт, он видит спокойный navy-интерфейс из разделов 1–6, а цвет продукта остаётся лишь маленькой точкой в переключателе.

**The One-Product-One-Color Rule.** Цвет закреплён за продуктом и не переходит на другой. Мятный `#2DC48D` по-прежнему только про деньги: на плитке «Счета» он стоит на печати оплаты, а не как декор.

### Don't
- **Don't** тянуть Unbounded, Caveat и радиус 28px в кабинеты.
- **Don't** красить кабинет цветом продукта. Кабинет всегда navy.
- **Don't** оформлять заблокированный продукт как рабочий: продукт, который ещё не готов (сейчас Wildberries), показывается с замком и подписью «скоро», без перехода внутрь.
- **Don't** добавлять новый цвет без нового продукта: палитра растёт только вместе с продуктами.
